...
class PremiumStatusNotificationService : Service() {
    ...
    private fun buildCompactLine(
        ...
        if (showFree) {
            lines.add("▸ ${boldUnicode("${storage.freeGb} GB")} LIVRE")
        }
        ...
        if (showRam) {
            bottomParts.add("RAM ${boldUnicode("${ram.freeGb}/${ram.totalGb} GB")} LIVRE")
        }
    ...
    private fun buildDetailedBody(
        ...
        if (showRam) {
            lines.add("RAM: ${boldUnicode("${ram.freeGb}/${ram.totalGb} GB")} LIVRE")
        }
